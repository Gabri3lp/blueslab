...
OUTPUT_DIR = Path("src/BluesLab/wwwroot")
...
def main():
    print("=== Blues Lab: PMEX Data Extraction Pipeline (Refined) ===")
    ...
    def get_move_name(move_id):
        return lsd["move_name_en"].get(str(move_id), f"Move #{move_id}").strip()
    ...
    for t in valid_trainers:
        ...
        t_name = t_verbose if t_verbose else lsd["trainer_name_en"].get(trainer_base_id, f"Trainer #{trainer_base_id}").strip()
        m_name = lsd["monster_name_en"].get(monster_base_id, f"Monster #{monster_base_id}").strip()
        ...
        for i in range(1, 5):
            ...
            if mid > 0:
                ...
                cat_num = mv.get("category", 3)
                category = "Physical" if cat_num == 1 else ("Special" if cat_num == 2 else "Status")
                moves.append({
                    "id": mid,
                    "slot": i,
        ...
        for cid, ap in dedup_panels.items():
            ...
            if 1 <= ab_type_num <= 6: # Stat boost
                color_kind = "stat"
                stat_key, stat_label = STAT_NAMES[ab_type_num]
                title = f"{stat_label} +{val}"
                desc = f"Raises {stat_label} by {val}."
                stat_bonus[stat_key] = val
            elif ab_type_num == 7: # Passive
                color_kind = "passive"
                title = get_passive_name(pid)
                desc = get_passive_desc(pid)
            elif ab_type_num == 8: # Move effect
                color_kind = "move effect"
                mname = get_move_name(mid) if mid else "Move"
                pname = get_passive_name(pid) if pid else ""
                title = f"{mname}: {pname}" if pname else mname
                desc = get_passive_desc(pid) if pid else f"Additional effect for {mname}."
            elif ab_type_num in [9, 10]: # Move power/accuracy boost
                mv_obj = moves_dict.get(str(mid), {})
                is_sync_boost = mv_obj.get("group") == "Sync" or mid == sync_mid
                mname = get_move_name(mid) if mid else sync_move_name
                if is_sync_boost:
                    color_kind = "sync"
                    ...
                    color_kind = "move boost"
                    title = f"{mname}: Power +{val}"
                    power_bonus[mname] = val
                    desc = f"Raises the power of {mname}."
                else:
                    color_kind = "move boost"
                    title = f"{mname}: Accuracy +{val}"
                    desc = f"Raises the accuracy of {mname}."
            elif ab_type_num == 11: # Learn move
                color_kind = "learn move"
                mname = get_move_name(mid) if mid else "Move"
                title = mname
                desc = get_move_desc(mid) if mid else "Unlocks move."
