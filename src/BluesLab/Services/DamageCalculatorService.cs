    ...
    public List<ActiveThemeSkillInfo> GetActiveThemeSkills(CombatantState ally, TeamBattleState? team)
    {
        ...
        if (hasMultiplePairsInTeam && team != null)
        {
            ...
            if (allTeamThemeIds.Count > 0)
            {
                foreach (var thId in allTeamThemeIds)
                {
                    ...
                    if (matchingAllies.Count >= 2)
                    {
                        ...
                        long group = thId / 10000;
                        bool isType = group == 2001;
                        bool isSpec = group < 2004; // Region (2002) and Trainer Groups (2003: Rival, Champion, Main Character, Gym Leader, Elite Four, Villain, etc.)

                        string themeName = ThemeDefinitionsMap.TryGetValue(thId, out var def) ? def.Name : $"Theme #{thId}";
                        string category = isType ? "Type" : (group == 2002 ? "Region" : "Tag");

                        // A Type theme skill only applies to sync pairs of that type
                        if (isType && !string.Equals(themeName, ally.Pair.Type, StringComparison.OrdinalIgnoreCase))
                        {
                            continue;
                        }
                        ...
                        foreach (var m in matchingAllies)
                        {
                            ...
                            if (isType)
                            {
                                ...
                                else if (isTech || isMulti) { atk += 24; spa += 24; hp += 24; }
                                else if (isSprint) { atk += 24; spa += 24; spe += 24; }
                                else if (isSupport) { hp += 60; }
                                else if (isField) { hp += 24; spe += 24; }
                            }
                            else if (isSpec) // Region (2002) & Trainer Groups (2003: Rival, Champion, etc.)
                            {
                                if (isStrike) { atk += 8; spa += 8; }
                                else if (isTech || isMulti) { atk += 4; spa += 4; hp += 7; }
                                else if (isSprint) { atk += 4; spa += 4; spe += 7; }
                                else if (isSupport) { hp += 13; }
                                else if (isField) { hp += 7; spe += 7; }
                            }
                            else // DESC (group >= 2004: Fashion, Arc Suit, Other tags like Pallet Town)
                            {
                                if (isStrike) { atk += 9; spa += 9; }
                                else if (isTech || isMulti) { atk += 5; spa += 5; hp += 8; }
                                else if (isSprint) { atk += 5; spa += 5; spe += 8; }
            ...
            if (!result.Any(r => r.Category == "Type"))
            {
                ...
                {
                    if (!string.Equals(group.Key, ally.Pair.Type, StringComparison.OrdinalIgnoreCase))
                        continue;
                    ...
                    if (matchingAllies.Count >= 2)
                    {
                        ...
                        foreach (var m in matchingAllies)
                        {
                            ...
                            else if (r.StartsWith("Sprint", StringComparison.OrdinalIgnoreCase))
                            {
                                atk += 24; spa += 24; spe += 24;
                            }
        ...
        // Theme skills require 2 or more sync pairs in the team to activate (cannot activate in solo mode).
        ...
        string? originalMoveType = null)
    {
        ...
        for (int i = 0; i < team.Allies.Count; i++)
        {
            ...
            if (teammate.SuperAwakeningLevel >= 5 && teammate.Pair.SuperAwakeningPassive != null && !string.IsNullOrEmpty(teammate.Pair.SuperAwakeningPassive.Name))
            {
                ...
                if (saRule != null)
                {
                    if (saRule.SubPassives != null && saRule.SubPassives.Count > 0)
                    {
                        foreach (var sp in saRule.SubPassives)
                        {
                            if (IsTeamWidePassive(sp))
                            {
                                ...
                                {
                                    total += v;
                                    pills.Add(new MultiplierPill { Label = $"Ally SA: {sp.Name} ({trainerName})", Value = $"+{v * 100:0.#}%", Color = "#8e44ad" });
                                }
                            }
                        }
                    }
            ...
            foreach (var p in passivesList)
            {
                ...
                if (rule != null)
                {
                    if (rule.SubPassives != null && rule.SubPassives.Count > 0)
                    {
                        foreach (var sp in rule.SubPassives)
                        {
                            if (IsTeamWidePassive(sp))
                            {
                                double v = EvalSingleDamagePassive(sp, move, activeAttacker, enemy, field, originalMoveType);
                                ...
                                {
                                    total += v;
                                    pills.Add(new MultiplierPill { Label = $"Ally: {sp.Name} ({trainerName})", Value = $"+{v * 100:0.#}%", Color = "#8e44ad" });
                                }
                            }
                        }
                    }
                ...
                // Also check childPassives from pair datamine if not already included in sub_passives
                if (p.ChildPassives != null && p.ChildPassives.Count > 0)
                {
                    foreach (var cp in p.ChildPassives)
                    {
                        if (rule?.SubPassives != null && rule.SubPassives.Any(sp => string.Equals(sp.Name, cp.Name, StringComparison.OrdinalIgnoreCase)))
                            continue;
                        ...
                        if (cpRule != null && IsTeamWidePassive(cpRule))
                        {
                            ...
                            {
                                total += v;
                                pills.Add(new MultiplierPill { Label = $"Ally: {cpRule.Name} ({trainerName})", Value = $"+{v * 100:0.#}%", Color = "#8e44ad" });
                            }
                        }
                    }
                }
                        ...
                        if (rule.SubPassives != null && rule.SubPassives.Count > 0)
                        {
                            foreach (var sp in rule.SubPassives)
                            {
                                if (IsTeamWidePassive(sp))
                                {
                                    ...
                                    {
                                        total += v;
                                        pills.Add(new MultiplierPill { Label = $"Ally Grid: {sp.Name} ({trainerName})", Value = $"+{v * 100:0.#}%", Color = "#8e44ad" });
                                    }
                                }
                            }
                        }
        ...
        List<MultiplierPill> pills)
    {
        ...
        if (ally.Pair != null)
        {
            ...
            foreach (var p in passivesList)
            {
                ...
                else if (p.ChildPassives != null && p.ChildPassives.Count > 0)
                {
                    foreach (var cp in p.ChildPassives)
                    {
                        var cpRule = rules.DamagePassives.FirstOrDefault(dp => string.Equals(dp.Name, cp.Name, StringComparison.OrdinalIgnoreCase));
                        if (cpRule != null)
                        {
                            ...
                            {
                                total += v;
                                pills.Add(new MultiplierPill { Label = cpRule.Name, Value = $"+{v * 100:0.#}%", Color = "#16a085" });
                            }
                        }
                    }
                }

                if (rule != null && p.ChildPassives != null && p.ChildPassives.Count > 0)
                {
                    foreach (var cp in p.ChildPassives)
                    {
                        if (rule.SubPassives != null && rule.SubPassives.Any(sp => string.Equals(sp.Name, cp.Name, StringComparison.OrdinalIgnoreCase)))
        ...
        foreach (var andGroup in conditionGroups)
        {
            ...
            foreach (var c in andGroup)
            {
                ...
                bool match = cond switch
                {
                    ...
                    "hp_half_more" or "hp_above_half" or "hp_half_or_more" => ally.HpPercent >= 50,
